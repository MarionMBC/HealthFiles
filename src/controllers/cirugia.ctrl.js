...
export const getCirugia = async (req, res) => {
  try {
    ...
    const [cirugia] = await pool.query(
      "SELECT * FROM cirugia WHERE codigo_cirugia= ?",
      [codigo]
    );
    if (cirugia.length <= 0) {
      //--Verificación de que el objeto no venga vacío
...
export const updateCirugia = async (req, res) => {
  try {
    ...
    const { dni_paciente, nombre, tipo, motivo, tratamiento, observaciones } =
      req.body;
    ...
    await pool.query(
      ...
      [dni_paciente, nombre, tipo, motivo, tratamiento, observaciones, codigo]
    );
...
export const deleteCirugia = async (req, res) => {
  try {
    const codigo = req.params.codigo_cirugia;
    const [cirugia] = await pool.query(
      "DELETE FROM cirugia WHERE codigo_cirugia=?",
      [codigo]
    );
    if (cirugia.affectedRows == 0) {
      //--Se verifica que hayan filas afectadas
      res.status(404).json({ msg: "No se encontró la cirugia" });
    } else {
      res
        .status(200)
        .json({ ok: true, msg: "Se ha eliminado correctamente la cirugia." });
    }
  } catch (error) {
    return res.status(500).json({
...
export const getCirugiasMedPac = async (req, res) => {
  try {
    ...
    const  [cirugias] = await pool.query(
      "SELECT cir.codigo_cirugia,concat(med.primer_nombre, ' ', med.primer_apellido) AS nombre_doctor,concat(p.primer_nombre, ' ', p.primer_apellido) AS nombre_paciente,med.dni_medico,p.dni_paciente,cir.tipo,cir.motivo,cir.tratamiento,mg.fecha,cir.observaciones FROM cirugia cir INNER JOIN medico_cirugia mg on mg.codigo_cirugia = cir.codigo_cirugia INNER JOIN medico med on med.dni_medico = mg.dni_medico INNER JOIN paciente p on p.dni_paciente = cir.dni_paciente WHERE med.dni_medico = ? and p.dni_paciente = ?",
    ...
    res.status(500);
    res.json({
      msg: `Error al obtener los registros de la tabla "medico_cirugia". Error: ${error}`,
    });
  }
};
